...
\par One network model we examine is the stochastic actor-oriented models introduced by Snijders~\cite{Snijders:1996}. Social networks are generally not static: they evolve over time and this evolution is influenced by the actors and the network structure. Estimation and implementation of these models can be done in the RSIENA software~\cite{RSiena}.  The software allows many possible effects to be included in this type of model, given below. XX insert math after I read the Snijders papers XX  Some of the network effects and actor effects we look at are XXX insert list of effects XXX. 
