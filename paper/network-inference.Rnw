...
\st{
One of the data sets provided with RSiena is a subset of 50 female respondants from the \emph{Teenage Friends and Lifestyle Study} XXX citation??? XXX. There are three friendship networks provided at three timepoints representing the relationships between the 50 teenage girls in the data. There are also actor-level variables observed at each time period, such as a girl's drinking and smoking behavior. These networks are shown in \autoref{fig:friendship_waves}
XX autoref not working?? XX
<<friendship_waves, echo=FALSE, message=FALSE, warning=FALSE>>=
