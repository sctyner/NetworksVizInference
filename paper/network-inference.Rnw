...
\par When given the opportunity to change or not, each actor tries to optimize its own objective function $f_i(\beta,x)$ under the current configuration of the network (denoted by $x$) in the observed time point.  The probability that actor $i$  changes a tie $x_{ij}$ \hh{to its opposite $x'_{ij}$ }  when given the opportunity to act is~\cite{Snijders:1996} \hh{could you first describe this function in words and then give the formula? It looks like a comparison of odds similar to a conditional logit model or a discrete choice model~\cite{benatkiva:1985}}
$$ 
p_{ij} = \frac{\exp(f(\beta,x| x_{ij}=x'_{ij}) - f(\beta, x |x_{ij}=x_{ij}))}{\sum_{h=1}^n \exp(f(\beta, x |x_{ih}=x'_{ih}) - f(\beta, x|x_{ih}=x_{ih}))}.
$$ 
\hh{Sam, could you check in the Snijder paper again about the actual form of the denominator? could it be two sums rather than the one?} }
