...
when given the opportunity to act, an actor tries to optimize its own objective function $f_i(\beta,x)$ under the current configuration of the network (denoted by $x$) in the observed time point.  The probability that actor $i$  changes a tie $x_{ij}$ \hh{to its opposite $x'_{ij}$} is~\cite{Snijders:1996} \hh{XXX could you first describe this function in words and then give the formula? It looks like a comparison of odds similar to a conditional logit model or a discrete choice model~\cite{benatkiva:1985}}
$$ 
