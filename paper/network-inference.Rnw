...
@
\caption{\label{fig:friendship_waves}\st{The friendship network of 50 girls in the \emph{Teenage Friends and Lifestyle Study} over three time periods. The nodes, which represent the girls in the study, are colored by their level of alcohol use.}}
\end{figure}
