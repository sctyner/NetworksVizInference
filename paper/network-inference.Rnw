...
\par Estimation and implementation of these models can be done in the RSIENA software~\cite{RSiena}.  The software allows many possible effects to be included in this type of model. Some of the network effects and actor effects we look at are: 
\begin{table}  
  \begin{tabular}{llc}
  Name & Type & Formula \\ 
  \hline
  period 1 rate & Rate & $\rho_1$ \\
