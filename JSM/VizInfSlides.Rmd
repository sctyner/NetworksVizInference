...
ggplot(data = dat %>% filter(plot_order2 %in% c(11,12)) %>% mutate(plot = plot_order2 - 10) ) + 
...
ggplot(data = dat %>% filter(plot_order %in% c(2,7)) %>% mutate(plot = ifelse(plot_order == 2, 1, 2))) + 
  geom_net(aes(from_id = X1, to_id = X2), size = 2, fiteach = T, directed = T, 
           colour = 'black', arrowsize = .25, arrowgap = .02, ecolour = 'grey60') + 
  facet_wrap(~plot) + 
  theme_net()
