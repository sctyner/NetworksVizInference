...
Broadly, a SAOM takes network structure and node covariate information into account in two ways: first through a rate function that describes \textit{when} changes in the network occur, and second through an objective function that describes \textit{what} those changes are. As in many other network models, the variables of interest, $x_{ij}$, are the binary edges of the network. For any two nodes $i \neq j$ in the network, $i,j \in \{1, 2, \dots, n=\text{the number of nodes}\}$, 
...
<<senateAll, fig.cap="The four senate collaboration networks that we use as our example data to visually assess the SAOM effects.">>=
...
For each senate session, we have the WPC value between any two senators in the session, the party affiliation of each senator, the number of bills they authored in each session, their age, and their sex. We explored each of these covariates in the model to determine if they affect the change of the network structure.  
...
In legislative cosponsorship networks, it is well known that party affiliation and reciprocity of relationships are major influences on structure \citep{legnet}.
...
We propose to attack some of the aforementioned difficulties with SAOMs through visual inference. 
...
We want to determine at which point an effect becomes noticeable in a SAOM. By \textit{noticeable}, we mean that the inclusion of the effect alters the appearance of networks simulated from a model. An effect becomes noticeable when as simulation from a model with a noticeable effect appears different from simulations from a model where the same affect is absent. Because the traditional way to visualize a network is a node-link diagram, we will examine the affect of different parameter values using node-link visualizations. 
