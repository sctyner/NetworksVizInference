...
We further explore this relationship between identification of the alternative data in the lineup and the parameter, effect size, and lineup type with a generalized linear mixed model that provides us with an estimate of the power of the visual test. The response variable, $Y_{ijkm}$, is binary, indicating whether participant $m$ picked the alternative data plot in lineup $j$, rep $k$, for effect $i$. The continuous covariate $x$ is the centered and scaled size of the effect of interest from which the alternative data were simulated, the values of which are labeled ``easy", ``medium", and ``hard" in Table~\ref{tab:experdetail} according to how difficult we thought the Turk participants would find each lineup. In Equation~\ref{eq:glmm}, $i \in \{1, 2, 3, 4, 6\}$ corresponds to the effects $\beta_1, \beta_3, \beta_4, \beta_2, \beta_6, \beta_5$, respectively. We include random effects in the model for each lineup, $\delta_{ijk}$ and each participant, $\epsilon_m$, and fit a hierarchical model as follows:
...
<<glmmres>>=
load("data/finalglmm31.RDA")
mod <- model1randomscalesize
res <- summary(mod)[[10]]
