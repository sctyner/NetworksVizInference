...
We use the estimates given in Table~\ref{tab:fittedvalues} to simulate from models M1 through M6. We discuss the simulation procedure and how we use the simulations in Section~\ref{sec:expersetup}.  
...
We want to explore the role of each of these parameters in the objective functions for each model. 
