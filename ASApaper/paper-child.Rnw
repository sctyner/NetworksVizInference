...
labdat <- data_frame(test_param = "jttp", type2 = -1, x = -5, y = .9, label = "outlier")
...
ggplot() + 
  ...
  geom_text(data = labdat, aes(x = x, y=y, label = label, color = as.factor(type2)),
            show.legend = F) + 
  scale_color_brewer(palette = "Dark2", name = "Lineup Type") + 
  facet_wrap(~test_param, scales = 'free_x') + 
...
ggplot() + 
  ...
  labs(x = "Parameter value (recip)", y = "Expected probability of detection in a lineup of size 6") + 
  ThemeNoNet + 
  theme(legend.position = 'bottom')
@

In Figure~\ref{fig:predictglmm} 
