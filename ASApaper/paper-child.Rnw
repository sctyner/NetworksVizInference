...
<<senateAll, fig.cap="The US senate collaboration network observed at four time points. Color represents party, shape represents gender, and size represents number of bills authored in a session. The Fruchterman-Reingold layout is shown \\citep{fr}. Drawn with the \\texttt{geomnet} R package. \\citep{geomnet, rstats}">>=
...
ggplot() + 
  ...
  geom_text(data = betahat, aes(x = size, y = .9, label = paste0("hat(beta)[", param, "]")), parse = T, hjust = 0, color = "grey60") + 
