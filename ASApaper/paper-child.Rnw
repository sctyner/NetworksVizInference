...
\end{equation}
dictating which edge node $i$ changes. Thus, the actor is more likely to make changes that increase the value of their objective function, and no change is most likely when any change decreases the value of the objective function.%Node $i$ can also choose to \textit{not} change at all, which is most likely when the value of Equation~\ref{eq:objective} is larger for $x$ than for any $x(i \leadsto j)$.  
...
\end{enumerate}
<<gofsiena, fig.height=2.5, fig.cap="An example of what a goodness-of-fit plot from \\texttt{RSiena} looks like. The overlaid boxplots and violin plots show the distribution of each of the outdegree count values on the simulated networks, and the red points and lines are the observed data values.", cache = TRUE, eval=FALSE>>=
