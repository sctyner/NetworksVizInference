...
And this difficulty only increases when studying \textit{dynamic networks}, the same set of nodes and their changing relationships observed at many points in time. Trying to determine the mechanism underlying change in a network over time only makes the model more complex. Yet, researchers are often interested in modeling dynamic social networks, such as friendship networks among students or collaboration networks between researchers. % X ST a bit less jarring hopefully. X 
