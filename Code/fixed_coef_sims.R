...
create_lu <- function(null_model, null_model_svs, alt_model, alt_model_svs, M, my_dat = mysmalldata, reps, yx = FALSE,...){
  for (r in 1:reps){
    if (yx == FALSE){
      lu_dat <- create_lu_dat_xy(null_model, null_model_svs, alt_model, alt_model_svs, m=M, my_dat, sd = r)
    } else lu_dat <- create_lu_dat_yx(null_model, null_model_svs, alt_model, alt_model_svs,m=M, my_dat, sd = r)
    ...
    savepdf(file = sprintf("Lineup-Images/pdfs/smallfriends-m-%s-rep-%s",M,r), width = 8*2.54, height = 8*2.54)
    print(lu_plot)
    dev.off()
