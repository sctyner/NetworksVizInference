...
fit_all_models <- function(dat){
  ...
  filename2 <- paste0("Data/ModelFitstoLineupData/",lineupname, "-", M, "-", rep, ".RDA")
...
for(i in 38:nrow(nested_lus)){
  fit_all_models_safely(dat = nested_lus$data[[i]])
}

fit_all_models_safely(dat = nested_lus$data[[388]])
