...
\hh{XXX Sam, when you are creating these lineups, could you split the functions into two, and create and save the data into a csv file in a first function, and then in a second function get the data and put it into a network? - That way we don't have to create the data again, if/when we change the design.} \st{done!}
