...
@
\caption{\label{fig:smallfriends}Adjacency matrices describing friendship relations between 50 students  in waves 1,2, and 3 of the friendship study \hh{XXX cite XXX}. The red squares identify the subset we focus on in the remainder. \hh{x and y axes labels are not particularly informative.}}
\end{figure}
