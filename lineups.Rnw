...
Effect name & Parameter & Corresponding Statistic & $M_1$ Estimate & $M_2$ Estimate & $M_3$ Estimate\\
\hline
\hline
...
set.seed(12345) # make shapes bigger, put labels in center. 
ggplot(data = jTT, aes(from_id = from, to_id = to)) + 
  geom_net(aes(shape = as.factor(group)), directed = T, label = T, labelcolour='blue',vjust = -1, hjust =2, arrowgap = .05, colour = 'black') + 
