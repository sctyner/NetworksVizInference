...
The two models we fit are a ``null model" (model $M_1$) and an ``alternative model" (model $M_2$). The null model only includes the default parameters that are included in the RSiena estimation. The alternative model includes one additional covariate parameter that was determined to be significant by the Wald test in the RSienaTest package.  Details on these effects are given in the table below.

\begin{table}[h]
\centering
\tiny
\begin{tabular}{lcccc}
Effect name & Symbol & Corresponding Statistic & $M_1$ Estimate & Model Y Estimate\\
\hline
\hline
Rate 1 (wave 1 $\rightarrow$ 2) & $\alpha_1$ & $\sum\limits_{i,j = 1 i\neq j}^n (x_{ij}(t_2) - x_{ij}(t_1))^2 $ & \Sexpr{round(lineup1$null_mod_ests[1], 3)} & \Sexpr{round(lineup1$alt_mod_estimates[1],3)}\\
Rate 2 (wave 2 $\rightarrow$ 3) & $\alpha_2$ & $\sum\limits_{i,j = 1 i\neq j}^n (x_{ij}(t_3) - x_{ij}(t_2))^2 $ & \Sexpr{round(lineup1$null_mod_ests[2], 3)} & \Sexpr{round(lineup1$alt_mod_estimates[2],3)}\\
Outdegree & $\beta_1$ & $s_{i1}(x) = \sum\limits_{j=1}^n x_{ij}$ & \Sexpr{round(lineup1$null_mod_ests[3], 3)} & \Sexpr{round(lineup1$alt_mod_estimates[3],3)}\\
Reciprocity & $\beta_2$ & $s_{i2}(x) = \sum\limits_{j=1}^n x_{ij}x_{ji}$ & \Sexpr{round(lineup1$null_mod_ests[4], 3)} & \Sexpr{round(lineup1$alt_mod_estimates[4],3)}\\
Jumping Transitive Triplets & $\beta_3$ & $s_{i3}(x) = \sum\limits_{\forall j\neq h} x_{ij}x_{ih}x_{hj} \mathbb{I}(v_i = v_h \neq v_j)$ & -- & \Sexpr{round(lineup1$alt_mod_estimates[5],3)}
\end{tabular}
\end{table}
...
\hh{It might be better to re-phrase the two functions below from `null' and `data' to model $M_1$ and model $M_2$, where $M_1$ is nested within model $M_2$. Both $M_1$ and $M_2$ are based on the data - so the distinction between null and data might add a bit of confusion, when we flip the models around in the next step.  }
  ...
  f^{alt}_{i}(x) & = \hat{\beta}_1^{alt}s_{i1}(x) +  \hat{\beta}_2^{alt}s_{i2}(x) + \hat{\beta}_3^{data}s_{i3}(x) 
  \end{align*}
