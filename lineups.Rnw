...
\begin{figure}
<<results0b, fig.width=8, fig.height=10, out.width='\\linewidth', echo = FALSE, warning = FALSE>>=
load("Data/lus_ests_truth.rda")
lus_ests_truth$param_name <- 
  factor(lus_ests_truth$param_name,
    levels = c("rate", "outdegree (density)", "reciprocity",
...
ggplot(data=lus_ests_truth %>% filter(!is.na(convergence))) +
  ...
  xlab("Difference between parameter estimate and true value") + 
    xlim(c(-15,15))
@
\caption{\label{fig:convergence-ests} Difference between parameter estimate and true value. Densities are drawn for both converged and non-converged data. Panels with a light grey background show model fits with data sampled from the same model. }
\end{figure}
