...
\hh{The pilot study consisted of an evaluation of a set of 20 lineups by 11 volunteers. For each of the model situations ($M_1$ vs $M_2$, $M_2$ vs $M_1$, $M_1$ vs $M_3$ and $M_3$ vs $M_1$) on lineup of size $m = 3, 6, 9, 12$ was used.}
...
qplot(label, weight=tally, fill=data_pick, data=dframe_res) +
  ...
  scale_fill_brewer("Data panel chosen", palette="Set1", na.value="grey80") 
@
...
\hh{Figure~\ref{fig:data-picks} shows barcharts of responses from the pilot study distinguishing the}
